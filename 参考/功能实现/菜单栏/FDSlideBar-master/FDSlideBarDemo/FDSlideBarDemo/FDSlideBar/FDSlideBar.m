...
-(void)setItemsTitle:(NSArray<NSString *> *)itemsTitle
{
    _itemsTitle = itemsTitle;
    [self setupItems];
}

-(void)setItemsButton:(NSArray<UIButton *> *)itemsButton
{
...
- (void)setupItems {
    ...
    //初始化slider
    _sliderView.frame = CGRectMake(firstItem.frame.size.width/4.0, self.frame.size.height - SLIDER_VIEW_HEIGHT, firstItem.frame.size.width/2.0, SLIDER_VIEW_HEIGHT);
}
...
//        animationGroup.animations = @[positionAnimation];
    ...
    rect.size.width = CGRectGetWidth(item.frame)/2.0;
